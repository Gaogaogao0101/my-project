<template>
   <div id="int">
       <div class="til" style="background:#C5161D;position:fixed;top:0;height:1rem;z-index:2;">
               <router-link to="/member"><div class="left"><img src="../../lib/img/ios/baifanhui@3x.png" alt=""></div></router-link>
               <div class="right"><span>我的积分</span><samp @click="ints">小组奖分</samp></div>
           </div>
       <div class="top">
       ...
       <div class="bot">
            <div class="declare" @click="postuser" v-if="usershow">申报个人积分</div>
...
export default {
    ...
    methods:{ 
        ...
        postuser(){
            ...
            var url=`freeter-api/withdraw/declareUserIntegral?token=${token}`;
        ...
        // 获取身份是否为小组长
        getnum(){
            var url="freeter-api/credits/isShowTeam?token="+this.token;
            this.$http.post(url).then(res=>{ 
               if(res.body.code==0){
        ...
        ints(){  
            if(this.isShow==1){
                this.$router.push({name:'integrals'})
        ...
        getlist(){
           ...
           var url=`freeter-api/credits/integralDetail?token=${token}&year=${year}&month=${month}`;
            this.$http.post(url).then(res=>{   
                if(res.body.code==0){
                    this.del=res.body.myIntegralDetail.reverse();  
                }else if(res.body.code==500){Toast(res.body.msg);}
            })
        }, 
          // 选择日期弹出
        getifdate(){    
         ...
         // 我的积分
        getint(){
            var url='freeter-api/credits/integral?token='+this.token; 
            this.$http.post(url).then(res=>{
                if(res.body.code==0){
                    this.inte=res.body.myIntegral;
...
#int{
 ...
 .top{
     ...
     margin-top: 1rem;
     background: #C5161D;
     background-image: url(../../lib/img/ios/intar-back-img@3x.png);
     background-size: 4.27rem 2.88rem;
    background-repeat: no-repeat;
    background-position:1.55rem .8rem;
    position: relative; 
 }
 .til{
     width: 100%;
