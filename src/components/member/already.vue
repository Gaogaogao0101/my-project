 ...
 <div id="already">
    ...
    <div class="nav">
        <ul class="navs">
            ...
            <router-link to="/already"><li class="ole">已确认<div class="hr"></div></li></router-link>
    ...
    <div class="inlie" v-if="ls.length==0"> 
     ...
     <div class="lists-b" v-for="(item,index) in ls" :key="index">
        <div class="time">
            <div class="time-left">已完成</div>
            <div class="time-right"><img src="../../lib/img/ios/del@3x.png" @click="detailist(item.id)" alt=""></div>
        </div>
        <div class="commodity">
            ...
            <div class="cou-pri">
                ...
                <div class="right-pic" style="width:1.5rem;height:70%;float:right;margin-top:.5rem;text-align:right;">
                    <small class="price-count"><small>￥</small>{{item.totalMoney00}}</small>
                </div>
            </div>
        </div>``
        <div class="but">
            <div class="cancel-b">再次购买</div> 
        </div>
    </div>   
 </div>  
</template>
...
.lists-b .time .time-left{
    width: 50%;
    float: left;
    color: #C20D12;
    font-size: .32rem;
}
.lists-b .time .time-right{
    width: 50%;
    float: right;
}
.lists-b .time .time-right img{
    width: .38rem;
    height: .38rem;
    float: right;
}
...
.lists-b .commodity .comm-img img{
    width: 1.3rem;
    height: .83rem;
    margin-top: .4rem; 
}
.lists-b .commodity .comm-til{
    line-height: .4rem;
    font-size: .32rem;
    color: #313131;
    margin-top: .05rem;
...
.lists-b .commodity .cou-pri .cp-count{ 
    height: .55rem; 
    margin-top: .15rem;
     position: absolute;
    left: 25%;
...
.lists-b .commodity .cou-pri .cp-count span{
    display: block;
    height: .53rem;
    /* width: 1.74rem; */
    padding-left: .15rem;
...
.lists-b .commodity .cou-pri .cp-count .sml{
    font-size: .28rem;
    color: #C20D12;
    margin-left: .2rem;
    line-height: .55rem;
...
.lists-b .commodity .cou-pri .right-pic .price-count{ 
    color: #C20D12;
    font-size: .32rem;
    font-weight: bold;
    margin-top: .1rem;
...
.lists-b .commodity .cou-pri .right-pic .price-count small{
    font-size: .24rem;
}
.lists-b .but{
    margin-top: .35rem;
