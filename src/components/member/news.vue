   ...
   <div class="top">
    ...
    <div class="content">
        ...
        <div class="na"> 
            ...
            <div class="nas name-imgs"><input style="width:4.2rem;word-break:keep-all;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" type="text" v-model="city" /><div @click="toAddress" style="color:#939393;font-size:.32rem;">请选择</div></div>
                <!-- <input type="text" v-model="city" @click="toAddress"  placeholder="请选择"/> <img src="../../lib/img/ios/fanhuiright@2x.png" alt=""></div> -->
        </div>
        <div class="na"> 
    ...
    <v-distpicker type="mobile" @selected='selected' v-show="addInp" static-placeholder></v-distpicker></div>
</div>    
</template>
<script>
    ...
            }else if(!this.username==null){
        ...
        addcalss(){
            var token=this.token;
                ...
                var url=`freeter-api/address/api/update?token=${token}&personName=${name}&id=${id}&personTel=${Tel}&province=${province}&city=${citys}&area=${areas}&detailedAddress=${address}&isDefault=${more}`;
            if (!/^((1[3,5,8][0-9])|(14[5,7])|(17[0,6,7,8])|(19[7]))\d{8}$/.test(self.newPhone)) { 
                Toast("请输入正确的电话号码"); 
            }else if(!this.username==null){
                Toast("用户名不能为空"); 
