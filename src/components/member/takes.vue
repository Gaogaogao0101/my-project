<template>
    <div id="take">
        ...
        <div class="top" style="z-index:4;position:fixed;top:0;width:100%;" @click="returns">
            <div class="til-left" ><img src="../../lib/img/ios/fanhuizuo@3x.png" alt=""></div>
        ...
        <!-- <router-link to="/new"> -->
        <div class="new" @click="detail(address.id=0)">新建收货地址</div>
        <!-- </router-link> -->
    </div>
...
export default {
    ...
    methods:{
        ...
        postdetail(id){ 
            ...
            var url=`freeter-api/address/api/info?token=${this.token}&id=${id}`;
            this.$http.post(url).then(res=>{  
                if(res.body.code==0){ 
            ...
            sessionStorage.setItem('selectaddres',2);
            this.$router.push({name:"sorders",params:{id}});  
        },
          // 获取不包邮地区
